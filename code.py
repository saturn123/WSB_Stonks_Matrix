...
def getstonksdata(stonk, profileid):
    ...
    matrixportal.set_background(cwd + "/WSB_background_"+str(profileid)+".bmp")
...
while True:
    for stonk in Stonks:
        print(stonk)
        try:
            getstonksdata(stonk, Stonks.index(stonk)+1) #the index will be used as bg
            print("after getstonksdata")
            time.sleep(1 * 10)
